...
async function triggerIngest(db: Database.Database, sourceId: number, content: string): Promise<void> {
  ...
  try {
    ...
    queries.insertLintWarning(null, 'ingest_error', `Failed to process raw source: ${error.message}`, 'error');
  }
}
...
export function createIngestRoutes(db: Database.Database): Router {
  ...
  router.post('/save', async (req: Request, res: Response) => {
    try {
      const { title, author, content, description, publishedAt, sourceUrl } = req.body;
  ...
  router.post('/upload', upload.single('file'), async (req: Request, res: Response) => {
    try {
      ...
      // Insert into database
      const sourceId = queries.insertRawSource(title, author || null, content, checksum, null, null, null);
