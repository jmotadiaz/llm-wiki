...
export class Queries {
  ...
  getRawSourceByChecksum(checksum: string) {
    const stmt = this.db.prepare('SELECT * FROM raw_sources WHERE checksum = ?');
    return stmt.get(checksum) as any;
  }

  getAllRawSources() {
    const stmt = this.db.prepare('SELECT * FROM raw_sources ORDER BY created_at DESC');
  ...
  getAllWikiPages() {
    const stmt = this.db.prepare('SELECT * FROM wiki_pages ORDER BY updated_at DESC');
    return stmt.all() as any[];
  }

  insertWikiPage(slug: string, title: string, content: string, type: string, tags?: string, status?: string) {
    const stmt = this.db.prepare(
      'INSERT INTO wiki_pages (slug, title, content, type, tags, status) VALUES (?, ?, ?, ?, ?, ?)'
    );
    const result = stmt.run(slug, title, content, type, tags || null, status || 'published');
    return result.lastInsertRowid as number;
  }

  updateWikiPage(id: number, title: string, content: string, tags?: string, status?: string) {
    const stmt = this.db.prepare(
  ...
  insertPageSource(pageId: number, sourceId: number) {
    const stmt = this.db.prepare('INSERT OR IGNORE INTO page_sources (page_id, source_id) VALUES (?, ?)');
  ...
  insertWikiLink(fromPageId: number, toPageSlug: string) {
    const stmt = this.db.prepare('INSERT OR IGNORE INTO wiki_links (from_page_id, to_page_slug) VALUES (?, ?)');
  ...
  // Lint Warnings
  insertLintWarning(pageId: number | null, type: string, message: string, severity?: string) {
    const stmt = this.db.prepare(
      'INSERT INTO lint_warnings (page_id, type, message, severity) VALUES (?, ?, ?, ?)'
    );
    const result = stmt.run(pageId || null, type, message, severity || 'warning');
    return result.lastInsertRowid as number;
  }

  getLintWarnings(pageId?: number) {
    if (pageId) {
      const stmt = this.db.prepare('SELECT * FROM lint_warnings WHERE page_id = ? AND resolved = 0');
      return stmt.all(pageId) as any[];
    }
    const stmt = this.db.prepare('SELECT * FROM lint_warnings WHERE resolved = 0 ORDER BY created_at DESC');
    return stmt.all() as any[];
  }

  resolveLintWarning(warningId: number) {
    const stmt = this.db.prepare('UPDATE lint_warnings SET resolved = 1 WHERE id = ?');
    stmt.run(warningId);
  }
